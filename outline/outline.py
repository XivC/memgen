...
def add_gradient_outline(image: Image.Image, outline_width: int = 100, color: tuple = (255, 255, 0)) -> Image.Image:
    """
    ...
    blurred_outline = outline_mask.filter(ImageFilter.GaussianBlur(radius=outline_width / 2))
    ...
    for y in range(gradient_outline.height):
        for x in range(gradient_outline.width):
            ...
            if alpha_value > 0:
                new_alpha = int(alpha_value * 255 / np.max(blurred_data))
